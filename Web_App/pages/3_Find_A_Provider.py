...
def getLocation():
    ...
    if loc != None:
        ...
        raise TypeError("Location Blocked")
...
def populateMarkers(df, m):
    for i in df['geom'].unique():
        ...
        folium.Marker([lat, lon], popup=popup, icon=folium.Icon(color='red', prefix='fa', icon='stethoscope'),
...
st.markdown(
    '<p class = "dynamicfont">This page allows you to find healthcare providers in your area who are can provide cervical cancer screenings and treatment. ',
    unsafe_allow_html=True)
col1, col2 = st.columns(2)
with col1:
    addr_input = st.text_input(label="Enter an Address Here:", key='addr', value="633 Clark St, Evanston, IL 60208",
                               on_change=toggle_addr)
...
if addr_input == "":
    ...
    try:
        ...
        df_show = df[
            ['distance', 'ProviderName', 'PrimarySpecialty', 'PracticePhoneNum', 'AddressFound', 'ProviderGender']]
        df_show['distance'] = df_show['distance'].round(2)
        df_show.columns = ['Distance (Miles)', 'Provider Name', 'Primary Specialty', 'Phone Number', 'Address',
                           'Provider Gender']
        st.markdown('#### Explore Providers Near You')
        st.write(
            "Providers who have registered a specialty as an OB/GYN, Family Medicine, or Internal Medicine physician are shown below, however, this may not be their primary specialty. Always contact the physician's office to confirm services offered.")
